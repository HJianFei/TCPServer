...
import manager.TcpClientManager;
...
public class TcpClient {
	...
	public synchronized TcpMsg sendMsg(TcpMsg msg) {
		if (isDisconnected()) {
			System.out.println("发送消息 " + msg + "，当前没有tcp连接，先进行连接");
			connect();
		}
	...
	private void onConnectSuccess() {
		System.out.println("tcp connect 建立成功");
	...
	private class ConnectionThread extends Thread {
		...
		public void run() {
			try {
				...
				System.out.println("创建连接成功,target=" + mTargetInfo + ",localport=" + localPort);
			} catch (Exception e) {
				System.out.println("创建连接失败,target=" + mTargetInfo + "," + e);
		...
		public void run() {
			try {
				...
				while (isConnected() && !Thread.interrupted()) {
					...
					if (result == null) {// 报错
						System.out.println("tcp Receive 粘包处理失败 " + Arrays.toString(result));
					...
					if (!va) {
						System.out.println("tcp Receive 数据验证失败 ");
				...
				System.out.println("tcp Receive  error  " + e);
	...
	private void notifyReceive(final TcpMsg tcpMsg) {
		for (TcpClientListener wl : mTcpClientListeners) {
			...
			wl.onReceive(TcpClient.this, tcpMsg);
	...
	public String toString() {
		return "XTcpClient{" + "mTargetInfo=" + mTargetInfo + ",state=" + mClientState + ",isconnect=" + isConnected()
				+ '}';
	}
}
